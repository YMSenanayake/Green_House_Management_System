...
const Products = () => {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          "http://localhost:3000/api/customeritems/getallitems"
        );
  ...
  const addToCart = async (item) => {
    try {
      await axios.post("http://localhost:3000/api/shoppingCart/addToCart", {
        name: item.name,
        _id: item._id,
        price: item.price,
        userid: "user123",
...
const scheduleOrder = async () => {
  ...
  try {
    ...
    const response = await axios.post("http://localhost:3000/api/ScheduleOrder/newScheduleOrder", result);
